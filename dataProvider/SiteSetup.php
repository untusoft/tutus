...
class SiteSetup
{
	...
	public function checkDatabaseCredentials(stdClass $params)
	{
		...
		if($success){
			...
			if($maxAllowPacket !== true){
				return array(
					'success' => false, 'error' => 'Could not set the MySQL <strong>max_allowed_packet</strong> variable.<br>Untusoft requires to set max_allowed_packet to 50M or more.<br>Please check my.cnf or my.ini, also you can install Untusoft using MySQL root user<br>max_allowed_packet = '.$maxAllowPacket
				);
			}
			return array(
	...
	public function checkRequirements()
	{
		try{
			...
			//		$row[]  = array(
			//			'msg' => 'Untusoft is not installed', 'status' => $status
			//		);
